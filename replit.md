...
- `/setleaderboardchannel` - Set channel where leaderboard panels will be posted
...
- `/addelo amount @user mode` - Add ELO to a player (1v1 or 2v2)
- `/removeelo amount @user mode` - Remove ELO from a player (1v1 or 2v2)
- `/updateleaderboard` - Post or update the leaderboard panels (pings top 10 players)
...
- `/leaderboard mode` - View top 10 ELO leaderboard (1v1 or 2v2)

## Leaderboard System
- Separate leaderboards for 1v1 and 2v2 modes
- Each player has separate ELO ratings for each mode
- Admins set a dedicated leaderboard channel with `/setleaderboardchannel`
- Hosts use `/updateleaderboard` to post/update both leaderboards
- Top 10 players are pinged when leaderboards are updated
- Leaderboard messages are edited (not recreated) when updated
